...
init_list = reactive({
  ...
  req(assignment(), validSeed, entry())
  ...
  validate(
    need(!assignment_data$exam || entry != "solve", 
...
}) %>% bindCache(assignment(), input$id, seed(), solutions(), assignment_accessible(), entry())
```
...
output$buttonBox <- renderUI({
  ...
  req(
    ...
    !assignment_data$exam || entry != "solve"
    )
