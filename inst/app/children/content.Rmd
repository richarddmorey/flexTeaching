...
}) %>% bindCache(assignment(), input$id, seed(), solutions(), assignment_accessible() || cache_mode(), entry(), seedMatches())
```
...
render_out <- reactive({
  ...
  future_promise({
    flexTeaching:::compilePage(assignment_data, id, seed, solutions, e, i) 
  }, 
  seed = NULL,
  )  %...>%
  withMathJax()
